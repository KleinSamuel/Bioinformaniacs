package dennis.genomeAnnotation;
...
public class GTFParser {
	...
	public static GenomeAnnotation readGtfFile(String name, String gtfFilePath, HashSet<String> chromosomesOfInterest) {
		GenomeAnnotation ga = new GenomeAnnotation(name);

		DebugMessageFactory.printInfoDebugMessage(true, "Started reading gtf-file: " + ga.getName());
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (chromosomesOfInterest != null) {
					if (!chromosomesOfInterest.contains(split[0])) {
						continue;
					}
				}
